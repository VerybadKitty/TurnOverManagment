...
fun TasksScreen(taskService: TaskService) {
    ...
    // Make sure to pass onCreateTask from the ViewModel to TasksContent
    TasksContent(
        tasksState = tasksState,
        isLoading = isLoading,
        errorMessage = errorMessage,
        onRefresh = { tasksViewModel.loadTasks() },
        onCreateTask = { tasksViewModel.createTask(it) }  // This is where you pass onCreateTask to the content
    )
}
...
fun TasksContent(
    ...
    errorMessage: String?,
    onRefresh: () -> Unit,
    onCreateTask: (Task) -> Unit  // Ensure this parameter is accepted here
) {
    var showDialog by remember { mutableStateOf(false) }

    if (showDialog) {
        // Correctly pass onCreateTask to AddTaskDialog
    ...
    AlertDialog(
        ...
        confirmButton = {
            Button(
                onClick = {
                    onCreateTask(Task(
                        id = "", // ID will be generated by the service or backend
